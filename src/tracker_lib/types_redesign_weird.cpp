...
struct vrschema
{
	...
	struct setting_subtable
	{
		setting_subtable(const char *const *tbl_in, int tbl_size_in, ALLOCATOR_DECL)
			...
			nodes(allocator)
		{
			nodes.reserve(tbl_size_in);
			for (int i = 0; i < tbl_size_in; i++)
			{
				// create an entry in the vector table per known property name
				nodes.emplace_back(tbl_in[i], allocator);
			}
		}
		//const char *const*tbl;	// the settings table is baked in
		//const int tbl_size;
...
{
	int index = m_context->m_resource_keys->GetApplicationsIndexer().get_index_for_key(key);
	if (index != -1)
	{
		*index_ret = index;
		return true;
...
vr::EVRApplicationError VRApplicationsCursor::GetApplicationKeyByIndex(uint32_t unExternalApplicationIndex, char * pchAppKeyBuffer, uint32_t unAppKeyBufferLen)
{
	...
	if (unExternalApplicationIndex < active_application_indexes->val.size())
	{
		...
		int count;
		const char *app_key = m_context->m_resource_keys->GetApplicationsIndexer().get_key_for_index(internal_index, &count);
...
bool GetIndexForSubSection(const char *settings_key, 
...
int *index)
{
	...
	{
		if (strcmp(settings_key, nodes[i].name) == 0)
		{
		*index = i;
		rc = true;
		break;
...
{\
	...
	if (GetIndexForSubSection(pchSettingsKey,\
		m_context->iterators->settings_node.sections[index].settings_field_name.nodes,\
		&subsection_index))\
	{\
					...
					char * Buffer, uint32_t Len, vr::EVRSettingsError * pError)
{
	...
	if (GetIndexForSection(pchSection, &index))
	{
		int subsection_index;
		if (GetIndexForSubSection(pchSettingsKey,
			m_context->iterators->settings_node.sections[index].string_settings.keys,
			&subsection_index))
		{
...
public:
	...
	void SynchronizeChildVectors()
	{
		while (iter_ref.models.size() < state_ref.models.size())
		{
			...
			int model_index = (int)iter_ref.models.size()-1;
