...
struct StringPool
{
	...
	char *AllocOne()
	{
		...
		{
			size_t last_index = string_pool.size() - 1;
...
struct HistoryNodeOrIterator : HistoryIterator
{
	...
	HistoryNodeOrIterator(ALLOCATOR_DECL)
	{}
...
struct HistoryNodeOrIterator<T, P, false> : HistoryNode
{
	...
	HistoryNodeOrIterator(ALLOCATOR_DECL)
		: item("", allocator)
	{
		assert(0); // should never happen in the field. just here because settings deserialization visitor start_vector thinks it might need it
	}
...
struct HistoryVectorNodeOrIterator : HistoryIterator
{
	...
	HistoryVectorNodeOrIterator(ALLOCATOR_DECL)
	{}
...
struct vrschema
{
	...
	struct eye_schema
	{
		...
		VDECL(distortion, bool, DistortionCoordinates_t);
	...
	struct system_controller_schema
	{
		system_controller_schema(ALLOCATOR_DECL)
			...
			string_props(allocator),
			bool_props(allocator),
			float_props(allocator),
			mat34_props(allocator),
			int32_props(allocator),
			uint64_props(allocator),
		...
		VVDECL(string_props, ETrackedPropertyError, char);
		VSDECL(bool_props, ETrackedPropertyError, bool);
		VSDECL(float_props, ETrackedPropertyError, float);
		VSDECL(mat34_props, ETrackedPropertyError, HmdMatrix34_t);
		VSDECL(int32_props, ETrackedPropertyError, int32_t);
		VSDECL(uint64_props, ETrackedPropertyError, uint64_t);
	...
	struct system_schema
	{
		...
		std::vector<spatial_sort_schema, ALLOCATOR_TYPE> spatial_sorts;  // indexes mean: 0 is absolute, +1 for relative to each controller index
	...
	struct application_schema
	{
		...
		VVDECL(string_props, EVRApplicationError, char);
		VSDECL(uint64_props, EVRApplicationError, uint64_t);
		VSDECL(bool_props, EVRApplicationError, bool);
	};
...
// traverses vr state to build a graph of grid_nodes for use by the gui
...
struct encoder_visitor : public lhs_only_visitor
{
	...
	inline void start_vector(const char *vector_name, T &vec)
	{
		// given a vector - encode it's int size onto the stream
		int size = (int32_t)vec.size();
		encode(size, *e);
	}
...
struct CompositorWrapper
{
	...
	inline void GetVulkanInstanceExtensionsRequired(vector_result<char> *result)
	{
		result->s.buf()[0] = 0;
		compi->GetVulkanInstanceExtensionsRequired(result->s.buf(), result->s.max_count());
		result->count = (int)strlen(result->s.buf()) + 1;
	}
...
struct OverlayWrapper
{
	...
	inline void GetKeyboardText(vector_result<char> *result)
	{
...
public:
	...
	void update_handle_for_index(int index, VROverlayHandle_t h)
	{
							...
							ALLOCATOR_DECL)
{
	...
	if (visitor.visit_source_interfaces())
	{
		...
		visitor.visit_node(ss->distortion.item, coords, rc, count);	
		wrap.FreeDistortion(coords);
	}
	else
	{
		visitor.visit_node(ss->distortion.item);
	}
...
template <typename visitor_fn, typename type_name, typename presence_type>
void visit_vec(visitor_fn &visitor,
	TrackedDeviceIndex_t unDeviceIndex,
	std::vector<HistoryNodeOrIterator<type_name, presence_type, false>, ALLOCATOR_TYPE> &vec,
	SystemWrapper &wrap,
	...
	const char* vector_name)
{
	...
	{
		scalar_result<type_name, ETrackedPropertyError> result;
		if (visitor.visit_source_interfaces())
		{
			...
			visitor.visit_node(vec[index].item, result);
		}
		else
		{
			visitor.visit_node(vec[index].item);
		}
	}
...
void visit_string_vec(visitor_fn &visitor,
	TrackedDeviceIndex_t unDeviceIndex,
	std::vector<HistoryVectorNodeOrIterator<type_name, presence_type, false>, ALLOCATOR_TYPE> &vec,
	SystemWrapper &wrap,
	PropertiesIndexer *indexer,
	PropertiesIndexer::PropertySettingType prop_type,
	const char* vector_name)
{
	...
	if (visitor.visit_source_interfaces())
	{
		...
		for (int index = 0; index < num_props; index++)
		{
			...
			visitor.visit_node(vec[index].item, result.s.buf(), result.result_code, result.count);
		}
	}
	else
	{
...
static void visit_controller_state(visitor_fn &visitor, 
	vrstate::system_controller_schema *ss,
	vrstate::system_schema *system_ss,
	...
	PropertiesIndexer::PropertySettingType prop_type, ALLOCATOR_DECL)
{
	...
	if (vec.size() < num_props)
	{
		...
		for (int index = 0; index < num_props; index++)
		{
			...
			vec.emplace_back(prop_name, allocator);
		}
		*structure_version += 1;
	}
}
			...
			const char* vector_name)
{
	...
	for (int index = 0; index < num_props; index++)
	{
		scalar_result<type_name, EVRApplicationError> result;
		if (visitor.visit_source_interfaces())
		{
...
template <typename visitor_fn>
void visit_application_state(visitor_fn &visitor, vrstate::applications_schema *applications,
	 ...
	 AdditionalResourceKeys &resource_keys, ALLOCATOR_DECL)
{
	...
	if (visitor.visit_source_interfaces())
	{
		...
		visitor.visit_node(ss->active_application_indexes.item, ptr, (int)active_indexes.size());
	}
	else
	{
	...
	SettingsIndexer::SectionSettingType setting_type, ALLOCATOR_DECL)
{
	...
	if (vec.size() < required_size)
	{
		...
		for (int i = vec.size(); i < required_size; i++)
		{
			...
			vec.emplace_back(field_name, allocator);
		}
		*structure_version += 1;
	}
}
...
void visit_subtable2(visitor_fn &visitor, 
	std::vector<HistoryNodeOrIterator<T, vr::EVRSettingsError, false>, ALLOCATOR_TYPE> &subtable,
...
void visit_string_subtable2(visitor_fn &visitor,
	std::vector<HistoryVectorNodeOrIterator<T, vr::EVRSettingsError, false>, ALLOCATOR_TYPE> &subtable,
									...
									AdditionalResourceKeys &config, ALLOCATOR_DECL)
{
	...
	if (visitor.visit_source_interfaces())
	{
		...
		visitor.visit_node(ss->instance_extensions_required.item, extensions.s.buf(), extensions.count);
	}
	else
	{
		visitor.visit_node(ss->instance_extensions_required.item);
	}
...
static void visit_permodelcomponent(
	visitor_fn &visitor, 
	vrstate::rendermodel_component_schema *ss,
					...
					uint32_t unRenderModelIndex, ALLOCATOR_DECL)
{
	...
	if (visitor.visit_source_interfaces())
	{
		if (visitor.reload_render_models() || ss->vertex_data.item.empty())
		{
			...
			visitor.visit_node(ss->texture_map_data.item, rubTextureMapData, rc, unWidth * unHeight * 4);
								...
								ALLOCATOR_DECL)
{
	...
	if (visitor.visit_source_interfaces())
	{
		...
		visitor.visit_node(ss->keyboard_text.item, keyboard_text.s.buf(), keyboard_text.count);		
	}
	else
	{
		visitor.visit_node(ss->keyboard_text.item);
	}
...
static tracker *construct_tracker()
{
	slab *the_slab = new slab(32 * 1024 * 1024);	// textures can be >= 16MB 
...
{
	tracker *ret = construct_tracker();
	ret->additional_resource_keys.Init(c);
	return ret;
}
...
bool util_char_to_return_buf_rc(const char *val, uint32_t required_size, char *pRet, uint32_t unBufferCount, uint32_t *rc)
{
	...
	uint32_t *rc)
{
	...
	return util_char_to_return_buf_rc(ptr, (uint32_t)p->size(), pRet, unBufferCount, rc);
}
...
struct CursorContext
{
	CursorContext(
		...
		vrstate *my_state,
		std::forward_list<FrameNumberedEvent, ALLOCATOR_TYPE> *my_events,
		AdditionalResourceKeys *resource_keys,
		ALLOCATOR_DECL)
		:
	...
	vrstate *state;
	std::forward_list<FrameNumberedEvent, ALLOCATOR_TYPE> *m_events;
	AdditionalResourceKeys *m_resource_keys;
...
public:
	...
	void SynchronizeChildVectors()
	{
		...
		while (iter_ref.eyes.size() < state_ref.eyes.size())
		{
			...
			int index = (int)iter_ref.eyes.size()-1;
		...
		for (int i = 0; i < iter_ref.controllers.size(); i++)
		{
			...
			{
				iter_ref.controllers[i].components.emplace_back(m_context->m_allocator);
			}

			while (iter_ref.controllers[i].bool_props.size() < state_ref.controllers[i].bool_props.size())
			{
				iter_ref.controllers[i].bool_props.emplace_back(m_context->m_allocator);
			}

			while (iter_ref.controllers[i].float_props.size() < state_ref.controllers[i].float_props.size())
			{
				iter_ref.controllers[i].float_props.emplace_back(m_context->m_allocator);
			}

			while (iter_ref.controllers[i].int32_props.size() < state_ref.controllers[i].int32_props.size())
			{
				iter_ref.controllers[i].int32_props.emplace_back(m_context->m_allocator);
			}

			while (iter_ref.controllers[i].uint64_props.size() < state_ref.controllers[i].uint64_props.size())
			{
				iter_ref.controllers[i].uint64_props.emplace_back(m_context->m_allocator);
			}
			while (iter_ref.controllers[i].mat34_props.size() < state_ref.controllers[i].mat34_props.size())
			{
				iter_ref.controllers[i].mat34_props.emplace_back(m_context->m_allocator);
			}
			while (iter_ref.controllers[i].string_props.size() < state_ref.controllers[i].string_props.size())
			{
				iter_ref.controllers[i].string_props.emplace_back(m_context->m_allocator);
			}
		}
...
int CLAMP(int min, int max, int sample)
{
	if (sample < min)
		return min;
	if (sample > max)
...
bool VRSystemCursor::ComputeDistortion(vr::EVREye eEye, float fU, float fV, struct vr::DistortionCoordinates_t * pDistortionCoordinates)
{
	...
	LOG_EXIT_RC(distortion->presence, "CppStubComputeDistortion");
}
...
bool VRSystemCursor::PollNextEvent(struct vr::VREvent_t * pEvent, uint32_t uncbVREvent)
{
	...
	if (m_context->last_event_frame_returned < m_context->current_frame)
	{
		...
		{
			if (iter->frame_number < m_context->last_event_frame_returned)
				break;
			prev = iter;
		}
		...
		if (prev != m_context->m_events->end() && 
			prev->frame_number > m_context->last_event_frame_returned && 
			prev->frame_number  <= m_context->current_frame)
		{
			m_context->last_event_frame_returned = prev->frame_number;
			if (pEvent)
			{
				*pEvent = prev->event;
			}
...
struct VRApplicationsCursor : public VRApplicationsCppStub
{
	...
	void SynchronizeChildVectors()
	{
		...
		for (int i = 0; i < iter_ref.applications.size(); i++)
		{
			...
			{
				iter_ref.applications[i].string_props.emplace_back(m_context->m_allocator);
			}

			while (iter_ref.applications[i].bool_props.size() < state_ref.applications[i].bool_props.size())
			{
				iter_ref.applications[i].bool_props.emplace_back(m_context->m_allocator);
			}

			while (iter_ref.applications[i].uint64_props.size() < state_ref.applications[i].uint64_props.size())
			{
				iter_ref.applications[i].uint64_props.emplace_back(m_context->m_allocator);
			}
...
{
	CursorContext *m_context;
	vrstate::settings_schema &state_ref;
	vriterator::settings_schema &iter_ref;
	SettingsIndexer *m_settings_indexer;
...
public:
	...
	void SynchronizeChildVectors()
	{
		...
		while (iter_ref.sections.size() < state_ref.sections.size())
		{
		...
		for (int index = 0; index < iter_ref.sections.size(); index++)
		{
				...
				j++)
			{
				iter_ref.sections[index].bool_settings.emplace_back(
					state_ref.sections[index].bool_settings[j].item.name,
				...
				j++)
			{
				iter_ref.sections[index].float_settings.emplace_back(
					state_ref.sections[index].float_settings[j].item.name,
					m_context->m_allocator);
			}
				...
				j++)
			{
				iter_ref.sections[index].int32_settings.emplace_back(
					state_ref.sections[index].int32_settings[j].item.name,
					m_context->m_allocator);
			}
				...
				j++)
			{
				iter_ref.sections[index].string_settings.emplace_back(
					state_ref.sections[index].string_settings[j].item.name,
					m_context->m_allocator);
			}
		}
	}
...
{\
	auto iter = iter_ref.sections[section_index].settings_field_name.nodes[subsection_index];\
	auto state =state_ref.sections[section_index].settings_field_name.nodes[subsection_index];\
	update_iter(iter, state, m_context->current_frame);\
\
	settings_error = iter.item->presence;\
		\
...
#define GET_SETTING2(setting_type, settings_field_name) \
SynchronizeChildVectors();\
vr::EVRSettingsError settings_error = vr::VRSettingsError_UnsetSettingHasNoDefault;\
int section_index;\
int subsection_index;\
...
bool VRSettingsCursor::GetBool(const char * pchSection, const char * pchSettingsKey, vr::EVRSettingsError * pError)
{
	...
	bool rc = false;
	GET_SETTING2(SettingsIndexer::SETTING_TYPE_BOOL, bool_settings);
...
int32_t VRSettingsCursor::GetInt32(const char * pchSection, const char * pchSettingsKey, vr::EVRSettingsError * pError)
{
	...
	int32_t rc = 0;
	GET_SETTING2(SettingsIndexer::SETTING_TYPE_INT32, int32_settings);
...
float VRSettingsCursor::GetFloat(const char * pchSection, const char * pchSettingsKey, vr::EVRSettingsError * pError)
{
	...
	float rc = 0.0f;
	GET_SETTING2(SettingsIndexer::SETTING_TYPE_FLOAT, float_settings);
...
struct VRCursor
{
	...
	{
		m_context.current_frame = framenumber;
		m_context.last_event_frame_returned = framenumber - 1;
	}

	int GetFrame()
	{
		return m_context.current_frame;
	}
...
void set_cursor_frame(vr_cursor_t h, int framenumber)
{
	...
	cursor->SetFrame(framenumber);
}

void advance_cursor_one_frame(vr_state_tracker_t tracker_handle, vr_cursor_t h)
{
	tracker *s = static_cast<tracker*>(tracker_handle);
	VRCursor *cursor = static_cast<VRCursor*>(h);
	int start_frame; int end_frame;
	get_frame_range(tracker_handle, &start_frame, &end_frame);

	int current_frame = cursor->GetFrame();
	if (current_frame < end_frame)
	{
		cursor->SetFrame(current_frame+1);
	}	
}

void set_cursor_to_end_frame(vr_state_tracker_t tracker_handle, vr_cursor_t h)
{
	int start_frame; int end_frame;
	get_frame_range(tracker_handle, &start_frame, &end_frame);
	set_cursor_frame(h, end_frame);
}

void set_cursor_to_start_frame(vr_state_tracker_t tracker_handle, vr_cursor_t h)
{
	int start_frame; int end_frame;
	get_frame_range(tracker_handle, &start_frame, &end_frame);
	set_cursor_frame(h, start_frame);
}
...
{
	tracker *s = static_cast<tracker*>(h);
	s->additional_resource_keys.UpdateNearFar(nearz, farz);
...
{
	tracker *s = static_cast<tracker*>(h);
	s->additional_resource_keys.GetSettingsIndexer().AddCustomSetting(section, SettingsIndexer::SETTING_TYPE_BOOL, key);
}

void update_vr_config_int32_setting(vr_state_tracker_t h, const char *section, const char *key)
{
	tracker *s = static_cast<tracker*>(h);
	s->additional_resource_keys.GetSettingsIndexer().AddCustomSetting(section, SettingsIndexer::SETTING_TYPE_INT32, key);
}

void update_vr_config_float_setting(vr_state_tracker_t h, const char *section, const char *key)
{
	tracker *s = static_cast<tracker*>(h);
	s->additional_resource_keys.GetSettingsIndexer().AddCustomSetting(section, SettingsIndexer::SETTING_TYPE_FLOAT, key);
}

void update_vr_config_string_setting(vr_state_tracker_t h, const char *section, const char *key)
{
	tracker *s = static_cast<tracker*>(h);
	s->additional_resource_keys.GetSettingsIndexer().AddCustomSetting(section, SettingsIndexer::SETTING_TYPE_STRING, key);
}
...
vr_state_tracker_t load_vrstate_from_file(const char *filename)
{
	...
	if (size > 0)
	{
		...
		if (file.read(buffer.data(), size))
		{
			...
			// read the resource keys before the state
			...
			// advance the stucture 'versions'
			s->m_state.system_node.structure_version++;
			s->m_state.applications_node.structure_version++;
			s->m_state.rendermodels_node.structure_version++;
			s->m_state.settings_node.structure_version++;
