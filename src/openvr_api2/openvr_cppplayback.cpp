...
static bool snapshot_playback_mode = true;
...
// use an interface proxy base class that passes along everything
// override the individual members.
PROXY_CALCSIZE

static void *vrapi_interfaces_playback[13] =
{
};

Trampoline system(tracker_cursor)
Trampoline compositor()

// the playback interface uses all of the cursor interfaces
// except it knows when to advance the cursor
//
// how to I insert a method

CppTrampoline
{
	// has handles
	// passes along those handles
}

class PlaybackSystem : CppTrampoline
{
	PlaybackSystem()
	{
...
class CppPlabackImpInterface : public ImpInterface
{
public:
	virtual void Init()
	{
	...
	struct vr::TrackedDevicePose_t * pTrackedDevicePoseArray, uint32_t unTrackedDevicePoseArrayCount)
{
   ...
   if (train_tracker)
   {
	   ...
	   tracker_cursor_interfaces.sysi->GetDeviceToAbsoluteTrackingPose(eOrigin, fPredictedSecondsToPhotonsFromNow, pTrackedDevicePoseArray, unTrackedDevicePoseArrayCount);
	   assert(memcmp(pTrackedDevicePoseArray, tmp, sizeof(vr::TrackedDevicePose_t) * unTrackedDevicePoseArrayCount) == 0);
	   free(tmp);
   }
	...
	vr::TrackedDeviceIndex_t unRelativeToTrackedDeviceIndex)
{
   ...
   uint32_t rc = interfaces.cpp_interfaces()->sysi->GetSortedTrackedDeviceIndicesOfClass(
...
void HandleEventPollResult(bool poll_rc, vr::VREvent_t * pEvent)
{
	// HandleEmptyQueue
	if (poll_rc == false)
	{
...
bool VRSystemCppPassalong::PollNextEvent(struct vr::VREvent_t * pEvent, uint32_t uncbVREvent)
{
   ...
   HandleEventPollResult(rc, pEvent);
	...
	vr::VRControllerState_t * pControllerState, uint32_t unControllerStateSize)
{
   ...
   bool rc = interfaces.cpp_interfaces()->sysi->GetControllerState(unControllerDeviceIndex,
	...
	struct vr::TrackedDevicePose_t * pTrackedDevicePose)
{
   ...
   bool rc = interfaces.cpp_interfaces()->sysi->GetControllerStateWithPose(
